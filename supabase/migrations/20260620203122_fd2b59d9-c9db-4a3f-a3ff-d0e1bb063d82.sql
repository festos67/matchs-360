DROP FUNCTION IF EXISTS public.save_framework_atomic(uuid, text, jsonb);

CREATE FUNCTION public.save_framework_atomic(p_framework_id uuid, p_name text, p_themes jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WHERE id = p_framework_id AND COALESCE(is_archived, false) = false;
  ...
  INSERT INTO public.competence_frameworks (name, club_id, team_id, is_template, is_archived, archived_at)
  VALUES (p_name, v_framework.club_id, v_framework.team_id, v_framework.is_template, false, null)
  ...
  LOOP
    ...
    VALUES (v_new_framework_id, COALESCE(NULLIF(v_theme->>'name', ''), 'Thématique'),
            v_theme->>'color', COALESCE((v_theme->>'order_index')::int, 0))
    ...
    LOOP
      ...
      VALUES (v_new_theme_id, COALESCE(NULLIF(v_skill->>'name', ''), 'Compétence'),
              NULLIF(v_skill->>'definition', ''), COALESCE((v_skill->>'order_index')::int, 0));
    END LOOP;
  END LOOP;
  ...
  INTO v_result
  FROM public.competence_frameworks f WHERE f.id = v_new_framework_id;

  RETURN v_result;
END;
$function$;

REVOKE ALL ON FUNCTION public.save_framework_atomic(uuid, text, jsonb) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.save_framework_atomic(uuid, text, jsonb) FROM anon;
GRANT EXECUTE ON FUNCTION public.save_framework_atomic(uuid, text, jsonb) TO authenticated, service_role;