...
AS $function$
  ...
  preferred_role_club AS (
    ...
    WHERE a.ok
      ...
      AND ur.role = 'coach'
      AND ur.club_id = p_preferred_club_id
      AND c.deleted_at IS NULL
    LIMIT 1
  ),
  admin_preferred_club AS (
    SELECT c.id AS club_id
    FROM public.clubs c
    CROSS JOIN authorized a
    WHERE a.ok
      AND public.is_admin(auth.uid())
      AND c.id = p_preferred_club_id
      AND c.deleted_at IS NULL
    LIMIT 1
  )
  SELECT CASE
    ...
    ELSE COALESCE(
      ...
      (SELECT club_id FROM preferred_role_club),
      (SELECT club_id FROM admin_preferred_club)
    )
  END
$function$;
