...
import { ArrowLeft, Plus, User, Star, Settings, FileText } from "lucide-react";
...
export default function TeamDetail() {
  ...
  const fetchTeamData = async () => {
    try {
      // Fetch team with club info
      const { data: teamData, error: teamError } = await supabase
        .from("teams")
        .select("*, club:clubs(name, primary_color)")
        .eq("id", id)
        .maybeSingle();
      ...
      const { data: membersData, error: membersError } = await supabase
        ...
        .select(`
          ...
          member_type,
          coach_role,
          profile:profiles(id, first_name, last_name, nickname, photo_url)
        `)
        .eq("team_id", id)
        .eq("is_active", true);
  ...
  return (
    <AppLayout>
      ...
      <Button
        variant="ghost"
        className="mb-6 -ml-2"
        onClick={() => navigate(`/clubs/${team.club_id}`)}
      >
      ...
      <div className="glass-card p-8 mb-8">
        <div className="flex items-center gap-8">
          <div
            className="w-24 h-24 rounded-2xl flex items-center justify-center text-3xl font-display font-bold"
            style={{
              background: `linear-gradient(135deg, ${teamColor} 0%, ${teamColor}88 100%)`,
              color: "white",
              boxShadow: `0 4px 24px -4px ${teamColor}40`,
            }}
          >
          ...
          <div className="flex-1">
            ...
            <p className="text-muted-foreground mt-1">
              {team.club?.name} • {players.length} joueur
              {players.length > 1 ? "s" : ""} • {coaches.length} coach
              {coaches.length > 1 ? "es" : ""}
            </p>
          </div>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {coaches.map((coach, index) => (
            ...
            >
              <CircleAvatar
                name={getMemberName(coach)}
                subtitle={coach.coach_role === "referent" ? "Coach Référent ★" : "Coach Assistant"}
                imageUrl={coach.profile.photo_url}
                color={teamColor}
                size="md"
                badge={
                  coach.coach_role === "referent" ? (
          ...
          {coaches.length === 0 && (
            <div className="col-span-full flex flex-col items-center justify-center h-32 glass-card">
              <p className="text-muted-foreground">Aucun coach assigné</p>
            </div>
          )}
        </div>
      </div>
        ...
        {players.length > 0 ? (
            ...
            {players.map((player, index) => (
              ...
              >
                <CircleAvatar
                  ...
                  imageUrl={player.profile.photo_url}
                  color={teamColor}
                  size="md"
                  onClick={() => navigate(`/players/${player.profile.id}`)}
                />
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col items-center justify-center h-48 glass-card">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Ajoutez des joueurs à cette équipe
            </p>
