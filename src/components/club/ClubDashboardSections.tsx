...
export const ClubDashboardSections = ({ clubId, onCreateTeam }: ClubDashboardSectionsProps) => {
  ...
  const { data: kpiCoachesCount, isLoading: loadingCoaches } = useQuery({
  ...
  const { data: kpiPlayersCount, isLoading: loadingPlayers } = useQuery({
  ...
  const { data: teamsList, isLoading: loadingTeamsList } = useQuery({
  ...
  const { data: coachesList, isLoading: loadingCoachesList } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (profiles || []).map((p) => {
        ...
        return { id: p.id, first_name: p.first_name, last_name: p.last_name, photo_url: p.photo_url, email: p.email, assignments };
      }).sort((a, b) => {
