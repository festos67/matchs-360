...
export interface Evaluation {
  ...
  /** Permet de repérer un auto-débrief rempli par un autre que le joueur (son représentant légal). */
  player_id?: string;
  talent?: string | null;
  ...
  const evaluationsQuery = useQuery({
    ...
    queryFn: async (): Promise<Evaluation[]> => {
      const { data, error } = await supabase
        .from("evaluations")
        .select(`
          id, name, date, deleted_at, framework_id, type, evaluator_id, player_id, talent,
